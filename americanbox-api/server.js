...
// Helper para ejecutar queries usando el pool
...
app.post('/api/admin/orders', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    console.log('🔍 Datos extraídos:', { guide, user_id, address_id, status, total, weight_lbs, provider_id, tracking_code, location, price_per_lb });
...
app.get('/api/admin/categories', async (_req,res)=>{
  try { ok(res,{ ok:true, items: await query('SELECT * FROM categories ORDER BY id DESC') }); }
...
app.post('/api/admin/categories', async (req,res)=>{
  try {
    ...
    const r = await query('INSERT INTO categories (name) VALUES (?)',[name]);
...
app.delete('/api/admin/categories/:id', async (req,res)=>{
  try { await query('DELETE FROM categories WHERE id=?',[+req.params.id]); ok(res,{ ok:true }); }
...
app.post('/api/admin/products', async (req,res)=>{
  try {
    ...
    const r = await query('INSERT INTO products (name, code, price, category_id) VALUES (?,?,?,?)',
...
app.put('/api/admin/products/:id', async (req,res)=>{
  try {
    ...
    await query('UPDATE products SET name=?, code=?, price=?, category_id=? WHERE id=?',
      [name, code, Number(price), category_id||null, id]);
    ok(res,{ ok:true });
  } catch(e){ console.error(e); fail(res,'Error update product',500); }
});
app.delete('/api/admin/products/:id', async (req,res)=>{
  try { await query('DELETE FROM products WHERE id=?',[+req.params.id]); ok(res,{ ok:true }); }
...
app.get('/api/admin/providers', async (_req,res)=>{
  try { ok(res,{ ok:true, items: await query('SELECT * FROM providers ORDER BY id DESC') }); }
...
app.post('/api/admin/providers', async (req,res)=>{
  try {
    ...
    const r = await query('INSERT INTO providers (tracking_code,name,address) VALUES (?,?,?)',
...
app.put('/api/admin/providers/:id', async (req,res)=>{
  try {
    const { tracking_code, name, address } = req.body||{};
    await query('UPDATE providers SET tracking_code=?, name=?, address=? WHERE id=?',
...
app.delete('/api/admin/providers/:id', async (req,res)=>{
  try { await query('DELETE FROM providers WHERE id=?',[+req.params.id]); ok(res,{ ok:true }); }
...
app.put('/api/admin/company', async (req,res)=>{
  try {
    ...
    await query(`UPDATE company_settings SET ${set} WHERE id=1`, vals);
    ok(res,{ ok:true });
...
function sanitizeCustomer(b = {}) {
  ...
  return {
    ...
    price_per_lb: (b.price_per_lb === '' || b.price_per_lb === undefined || b.price_per_lb === null) ? null : parseFloat(b.price_per_lb) || null,
  };
}
    ...
    try {
      ...
      const [rows] = await conn.query(
        ...
        SELECT c.id, c.names, c.email, c.image_url, c.dni, c.mobile, c.phone,
               ...
               c.price_per_lb, DATE_FORMAT(c.created_at,'%Y-%m-%d %H:%i:%s') as created_at
        FROM customers c
        ${where}
        ORDER BY c.id ASC
        LIMIT ? OFFSET ?`,
...
app.post('/api/admin/customers', requireAdmin, async (req,res)=>{
  try {
    ...
    const r = await query(`
      INSERT INTO customers
      (names,email,image_url,dni,mobile,phone,address,birthdate,gender,identification_type,send_email_invoice,price_per_lb,created_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW())
    `,[c.names,c.email,c.image_url,c.dni,c.mobile,c.phone,c.address,c.birthdate,c.gender,c.identification_type,c.send_email_invoice,c.price_per_lb]);
  ...
  try {
    ...
    await query(`
      UPDATE customers SET
        ...
        birthdate=?, gender=?, identification_type=?, send_email_invoice=?, price_per_lb=?
      WHERE id=?`,
      [c.names,c.email,c.image_url,c.dni,c.mobile,c.phone,c.address,c.birthdate,c.gender,c.identification_type,c.send_email_invoice,c.price_per_lb,id]
    );
    ok(res,{ ok:true });
...
app.get('/api/admin/vouchers', requireAdmin, async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM vouchers v ${where}`, paramsCount);
    const total = countResult[0]?.total || 0;
...
app.get('/api/admin/invoices', requireAdmin, async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM invoices i ${where}`, paramsCount);
    const total = countResult[0]?.total || 0;
...
app.delete('/api/admin/invoices/:id', requireAdmin, async (req, res) => {
  try { await query('DELETE FROM invoices WHERE id=?', [+req.params.id]); res.json({ ok:true }); }
...
app.post('/api/admin/invoices', requireAdmin, async (req,res)=>{
  try {
    ...
    const r = await query(`INSERT INTO invoices (number, client_name, register_date, due_date, subtotal, iva, discount, total, created_at) VALUES (?,?,?,?,?,?,?,?,NOW())`, [number, client_name, register_date||null, due_date||null, subtotal||0, iva||0, discount||0, total||0]);
    ok(res, { ok:true, id: r.insertId });
...
app.get('/api/admin/credit-notes', requireAdmin, async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM credit_notes cn ${where}`, paramsCount);
    const total = countResult[0]?.total || 0;
...
app.delete('/api/admin/credit-notes/:id', requireAdmin, async (req, res) => {
  try { await query('DELETE FROM credit_notes WHERE id=?', [+req.params.id]); res.json({ ok:true }); }
...
app.post('/api/admin/credit-notes', requireAdmin, async (req,res)=>{
  try {
    ...
    if (!number || !client_name) return fail(res,'Campos requeridos',422);
    const r = await query('INSERT INTO credit_notes (number, client_name, register_date, total, created_at) VALUES (?,?,?,?,NOW())', [number, client_name, register_date||null, total||0]);
    ok(res,{ ok:true, id: r.insertId });
...
app.get('/api/admin/accounts-receivable', requireAdmin, async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM accounts_receivable a ${where}`, paramsCount);
    const total = countResult[0]?.total || 0;
...
app.delete('/api/admin/accounts-receivable/:id', requireAdmin, async (req, res) => {
  try { await query('DELETE FROM accounts_receivable WHERE id=?', [+req.params.id]); res.json({ ok:true }); }
...
app.post('/api/admin/accounts-receivable', requireAdmin, async (req,res)=>{
  try {
    ...
    const r = await query('INSERT INTO accounts_receivable (invoice_number, client_name, sale_date, due_date, debt, balance, status, created_at) VALUES (?,?,?,?,?,?,?,NOW())', [invoice_number, client_name, sale_date||null, due_date||null, debt||0, balance||0, status||null]);
    ok(res,{ ok:true, id: r.insertId });
...
app.get('/api/admin/receipt-errors', requireAdmin, async (req, res) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) as total FROM receipt_errors re ${where}`, paramsCount);
    const total = countResult[0]?.total || 0;
...
app.delete('/api/admin/receipt-errors/:id', requireAdmin, async (req, res) => {
  try { await query('DELETE FROM receipt_errors WHERE id=?', [+req.params.id]); res.json({ ok:true }); }
...
app.post('/api/admin/receipt-errors', requireAdmin, async (req,res)=>{
  try {
    ...
    const r = await query('INSERT INTO receipt_errors (invoice_number, register_date, type, stage, environment, errors, created_at) VALUES (?,?,?,?,?,?,NOW())', [invoice_number||null, register_date||null, type||null, stage||null, environment||null, errors||null]);
    ok(res,{ ok:true, id: r.insertId });
...
app.get('/api/admin/orders/:id/print-label', requireAdmin, async (req, res) => {
  try {
    ...
    const conn = await pool.getConnection();
    try {
      console.log('📊 Executing query for order ID:', id);
          ...
          <div class="label">
            ...
            <div class="header">
              <div class="logo-container">
                <div class="logo">
                  <img src="/logo-abox.png" alt="American Box" style="width: 100%; height: 100%; object-fit: contain;" />
                </div>
              </div>
      ...
      res.json({ ok: true, html: labelHtml });
